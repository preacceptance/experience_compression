...
PerformExclusions <- function(data) {
    ...
    data$attention_check <- ifelse(((data$att_check_1 == '2') &
        (data$att_check_2 == '2')), 0, 1)
...
##================================================================================================================
GetMainEffects <- function(data, data_long, n_plots, plot_names, data_plot_long) {
    "
    ...
    # Calculate r2 of the mixed effects model
    print('R2 of the mixed effects model:')
    rsq <- r.squaredGLMM(effect_mod)[2]
    print(rsq)

    # Using the r2 value, calculate the f2 effect size
    print('f2 effect size:')
    f2 <- rsq / (1 - rsq)
    print(f2)
...
pdf(file = "lifelines_bar_plot_pilot.pdf", width = mm_to_in(180), height = mm_to_in(85))
...
main_effects <- GetMainEffects(dat, data_long, n_plots, plot_names, data_plot_long)
pdf(file = "linear_vs_quadratic_fit.pdf", width = mm_to_in(180), height = mm_to_in(150))
plot(main_effects)
main_effects
dev.off()

data_long$index <- 1:nrow(data_long)
get_plot_index <- function(row) {
    return(data_long[data_long$plot_names == row['plot_names'] &
                            data_long$question_type == 'meaning_score_avg', 'index'])
}

dat$plot_type_n <- apply(dat, 1, get_plot_index) #create numeric version of plot_names
dat$score_n <- as.numeric(dat$score) #create numeric version of score (which are characters)
dat$question_type_n <- as.numeric(factor(dat$question_type))
dat$subject_n <- as.numeric(factor(dat$subject))

print('Did answers vary depending on question and plot type?')
effect_mod <- lmer(score_n ~ question_type_n * plot_type_n + (1 | subject_n), data = dat)
print(summary(effect_mod))
print('-----------------------------------------------------')
